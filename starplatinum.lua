...
-- all i did was do stuff to the reanim and do extra hat alignment - sametcetinkaya
...
-- Artificial Heartbeat BindableEvent
local ArtificialHB = Instance.new("BindableEvent")
ArtificialHB.Name = "Heartbeat"

-- Settings
local frame = 1 / 30 -- ~30 FPS target (0.0333s)
local tf = 0
...
--	print("Artificial Heartbeat tick (~30fps)")
end)
...
local myaircraft = workspace:FindFirstChild(myname .. " Aircraft")
...
for _, v in ipairs(myaircraft:GetDescendants()) do
    if v:IsA("BasePart") then
        v.CanCollide = false
        v.Massless = true
    end
    ...
    -- Limb Posts
    if v.Name == "Post" and v:IsA("Part") and v.Color == Color3.fromRGB(85,0,160) then
        table.insert(limbparts, v)

    -- Head
    elseif v.Name == "cylinder1x1" and v:IsA("MeshPart") and v.Color == Color3.fromRGB(85,0,160) then
        headpart = v

    -- Torso
    elseif v.Name == "BlockStd" and v:IsA("Part") then
        torsopart = v
    end
end

print("Limb parts:", #limbparts)

local st = workspace[myname]["Advancing Fortress"]

local targets = {
    st:FindFirstChild("Left Leg"),
    st:FindFirstChild("Right Leg"),
    st:FindFirstChild("Left Arm"),
    st:FindFirstChild("Right Arm"),
}

local headTarget = st:FindFirstChild("FHead")
local torsoTarget = st:FindFirstChild("FTorso")

local headCF = CFrame.new(0,0,0) * CFrame.Angles(math.rad(90),0,math.rad(-90))
local torsoCF = CFrame.new(0,0,0) * CFrame.Angles(math.rad(90),0,0)

RunService.RenderStepped:Connect(function()
    -- Limbs
    for i, part in ipairs(limbparts) do
    ...
    -- Head
    if headpart and headTarget then
        headpart.CFrame = headTarget.CFrame * headCF
    end

    -- Torso
    if torsopart and torsoTarget then
        torsopart.CFrame = torsoTarget.CFrame * torsoCF
    end
end)
