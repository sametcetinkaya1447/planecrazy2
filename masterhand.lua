	...
	local sametbabaAircraft = playerAircraft
	...
	local balls = {}
	for _, model in ipairs(sametbabaAircraft:GetDescendants()) do
	...
	for i,v in pairs(sametbabaAircraft:GetChildren()) do
		if v:IsA("Model") and v.Name == "Camera" and v.BlockStd.Color == Color3.fromRGB(10, 0, 0)  then
			beam1 = v.BlockStd
	...
	for i,v in pairs(sametbabaAircraft:GetChildren()) do
		if v:IsA("Model") and v.Name == "Camera" and v.BlockStd.Color == Color3.fromRGB(11, 0, 0)  then
			beam2 = v.BlockStd
	...
	for i,v in pairs(sametbabaAircraft:GetChildren()) do
		if v:IsA("Model") and v.Name == "Camera" and v.BlockStd.Color == Color3.fromRGB(12, 0, 0)  then
			beam3 = v.BlockStd
		end
	end
	for i,v in pairs(sametbabaAircraft:GetChildren()) do
		if v:IsA("Model") and v.Name == "Camera" and v.BlockStd.Color == Color3.fromRGB(13, 0, 0)  then
			beam4 = v.BlockStd
		end
	end
	for i,v in pairs(sametbabaAircraft:GetChildren()) do
		if v:IsA("Model") and v.Name == "Camera" and v.BlockStd.Color == Color3.fromRGB(14, 0, 0)  then
			beam5 = v.BlockStd
		end
	end
   ...
   local function align(Part0, Part1, res)
	...
	local function removeattachemnts(part)
		for _, attachment in ipairs(part:GetChildren()) do
			if attachment:IsA("Attachment") or attachment:IsA("AlignOrientation") or attachment:IsA("AlignOrientation") then
				attachment:Destroy()
			end
		end
	end
   ...
   --m.Transparency = 1
    ...
    Palm.Transparency = 1
    pPoint1.Transparency = 1
    pPoint2.Transparency = 1
    pPoint3.Transparency = 1
    
    pMid1.Transparency = 1
    pMid2.Transparency = 1
    pMid3.Transparency = 1

    pRing1.Transparency = 1
    pRing2.Transparency = 1
    pRing3.Transparency = 1

    pPinkie1.Transparency = 1
    pPinkie2.Transparency = 1
    pPinkie3.Transparency = 1

    pThumb1.Transparency = 1
	...
	local handFolder = workspace.Hand
	...
	local tip1, tip2, tip3, tip4, tip5 = handFolder:GetChildren()[31], handFolder:GetChildren()[25], handFolder:GetChildren()[19], handFolder:GetChildren()[13], handFolder:GetChildren()[7]
	function ShortifiedAnim(theanim)
		...
		repeat
			...
			tracker.Position = mouse.Hit.p
			tracker.Color = Color3.new(1, 0, 0)
			tracker.Transparency = 0.3
			tracker.Material = "Neon"
			...
			tracker.Parent = workspace
				...
				beam1:PivotTo(tip1.CFrame)
				beam2:PivotTo(tip2.CFrame)
				beam3:PivotTo(tip3.CFrame)
				beam4:PivotTo(tip4.CFrame)
				beam5:PivotTo(tip5.CFrame)
				wait(0.2)
				beam1:PivotTo(tracker.CFrame)
				beam2:PivotTo(tracker.CFrame)
				beam3:PivotTo(tracker.CFrame)
				beam4:PivotTo(tracker.CFrame)
				beam5:PivotTo(tracker.CFrame)
				wait(0.2)
				tracker:Destroy()
	...
	hb.Selected:connect(function(mouse)
		...
		local sametbabaAircraft = sametbabaAircraft
		...
		local function teleportBall(ball)
			...
			bulletVelocity.Velocity = direction * 1300
		...
		mouse.Button1Down:connect(function()
			...
			elseif Anim.a == "GroundGrip" then
				...
				local part = ro
				local transparency = 0.5 -- replace if you want 

				part.Transparency = transparency
				local toprak =  workspace.Hand.Rock
				align(redPart, toprak, 40)
	...
	local aircraft = sametbabaAircraft
    ...
    local palmrot = CFrame.Angles(math.rad(0), math.rad(0), math.rad(0))
    align(bluePart, Palm, 100)


local function findAndAlignPart(color, handPartIndex)
    local part = nil
    for _, blockStdModel in ipairs(aircraft:GetDescendants()) do
        if
            blockStdModel.Name == "BlockStd"
            and blockStdModel:IsA("Part")
            and blockStdModel.Color == Color3.fromRGB(unpack(color))
        then
            part = blockStdModel
            break
        end
    ...
    if not part then
        warn("Part with color", color, "not found!")
        return
    end

    local handPart = handFolder:GetChildren()[handPartIndex]
    local yRotation = CFrame.Angles(0, 0, 0)  -- 90 degrees on Y axis
    align(part, handPart, 100)
end


findAndAlignPart({ 161, 161, 161 }, 3) -- Yellow
findAndAlignPart({ 162, 162, 162 }, 5) -- Green
findAndAlignPart({ 163, 163, 163 }, 7) -- Turquoise
findAndAlignPart({ 164, 164, 164 }, 9) -- Dark Blue
findAndAlignPart({ 165, 165, 165 }, 11) -- Pink
findAndAlignPart({ 166, 166, 166 }, 13) -- Brown
findAndAlignPart({ 167, 167, 167 }, 15) -- Magenta
findAndAlignPart({ 168, 168, 168 }, 17) -- Lime
findAndAlignPart({ 169, 169, 169 }, 19) -- Purple-Blue
findAndAlignPart({ 170, 170, 170 }, 21) -- Dark Blue
findAndAlignPart({ 171, 171, 171 }, 23) -- Pale Red
findAndAlignPart({ 172, 172, 172 }, 25) -- Red
findAndAlignPart({ 173, 173, 173 }, 27) -- Orange
findAndAlignPart({ 174, 174, 174 }, 29) -- Phosphor Green
findAndAlignPart({ 175, 175, 175 }, 31) -- Powder Pink
   ...
   -- local sametbabaAircraft = sametbabaAircraft
	...
	local playerName = game.Players.LocalPlayer.Name
	local sametbabaAircraft = workspace[playerName.." Aircraft"]

	for _, model in ipairs(sametbabaAircraft:GetDescendants()) do
		if model:IsA("Model") and (model.Name == "HalfBall" or model.Name == "Cylinder" or model.Name =="BlockBeam2" or model.Name =="BlockBeam" or model.Name == "SplitCylinder 2x2" or model.Name == "BlockStd")  then
			for _, part in ipairs(model:GetDescendants()) do
				if (part:IsA("Part") or part:IsA("MeshPart") or part:IsA("UnionOperation")) and not part.Anchored then
